...
function drawSecondaryTable(arr) {
    ...
    let content = "";
    arr.forEach((data, i) => {
        content += `<div data-value='${converObjectToString(data)}' onClick='drawModal(this)' onmouseover="mouseOverElement(this)" style='${data.style
            ? `${data.style} background-color : ${pickColor(data.groupBlock)}`
            : `background-color : ${pickColor(data.groupBlock)}`}' class='element'>${drawElement(data)}</div>`;
    });
...
function drawMainTable(arr) {
    ...
    arr.forEach((data) => {
        data.forEach((data, i) => {
            content += `<div data-value='${converObjectToString(data)}' onClick='drawModal(this)' onmouseover="mouseOverElement(this)"  style='${data.style
                ? `${data.style} background-color : ${pickColor(data.groupBlock)}`
                : `background-color : ${pickColor(data.groupBlock)}`}' class='element'>${drawElement(data)} ${data.class ? `<h5 class="classElements">${data.class}</h5>` : ""}</div>`;
        });
    });
    div.innerHTML = content;
...
function drawHoveredElement(element) {
    return `
  <div >
  <span class.atomNumber>${element.atomicNumber}<h5 style="position: absolute; top: 0; left: -15ch; color: white;">Nomor Atom &rarr;</h5> </span>
  <span class="atomicMass">${element.atomicMass}<h5 style="position: absolute; top: 0; right: -15ch; color: white;"> &larr; Massa Atom</h5> </span>
  </div> 
  <div class="info">
   <span class="oxidationStates">${element.oxidationStates}<h5 style="position: absolute; top:  0; right: -19ch; color: white;"> &larr; Tingkat oksidasi</h5> </span>
    <div class="main-info">
      <div>
      <span class="boilingPoint">${element.boilingPoint}<h5 style="position: absolute; top: 0;left: -15ch; color: white;">Titik didih (C) &rarr;</h5> </span></span>
      <span class="meltingPoint">${element.meltingPoint}<h5 style="position: absolute; top: 0; left: -15ch; color: white;">Titik leleh (C) &rarr;</h5> </span></span>
      <span class="density">${element.density}<h5 style="position: absolute; top: 0; left: -15ch; color: white;">Massa Jenis  &rarr;</h5> </span></span>
      </div>
      <span class="symbol">${element.symbol}</span>
      <h5  style="font-size: 0.9em; position: absolute; top: 50%; right: -11ch; color: white;">&larr; Symbol</h5>
    </div>
     <span class="electronConfiguration">${element.electronicConfiguration}<h5 style="position: absolute; top: 0; right: -19ch; color: white;"> &larr; Struktur elektron</h5> </span>
  <span class="name">${element.name}<h5 style="position: absolute; top: 0; right: -15ch; color: white;"> &larr; Nama unsur;</h5></span>
  </div>
    `;
}
/*
